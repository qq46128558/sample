...
def get_page_num(size,count):
    """ 计算要抓取的页数 """
    res=math.ceil(count/size)
    # 拉勾网最多显示30页结果
    return 30 if res>30 else res
...
def main():
    ...
    # 先设定页数为1,获取总的职位数  
    page1=get_json(url,1)
    total_count = page1['content']['positionResult']['totalCount']
    page_size=page1['content']['pageSize']
    num=get_page_num(page_size,total_count)
    total_info=[]
    # 每次抓取完成后,暂停一会,防止被服务器拉黑
    # time.sleep(20)
    print('职位总数:{},页数:{}'.format(total_count,num))
    # 把第一页的加进去,不重复爬
    jobs_list=page1['content']['positionResult']['result']
    ...
    df.to_csv('lagou-jobs.csv',index=False,encoding='gb18030')
    print('已保存为csv文件.') 
