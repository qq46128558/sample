...
CheckStatus;
####add by hzk 06-05
printf "准备配置：兑奖金额和线下回款余额更新的定时任务数据......";
mysql -h localhost -u root -p${password} -D ${database} -e"INSERT INTO \`sdb_base_crontab\` (id,description,enabled,\`schedule\`,last,app_id,class,type) select 'b2c_tasks_updatemembers', '兑奖金额和线下回款余额的今年变更为去年并清零今年的值', 'false', '10 0 1 1 *', UNIX_TIMESTAMP(), 'b2c', 'b2c_tasks_updatemembers', 'custom' from dual where not exists (select 1 from \`sdb_base_crontab\` where id='b2c_tasks_updatemembers');" >>insert_predata.sh.log 2>&1
CheckStatus;
####add end
#add by peter 2017-06-06
printf "准备写入U8订单付款状态同步定时任务数据......";
mysql -h localhost -u root -p${password} -D ${database} -e"INSERT INTO \`sdb_base_crontab\` (id,description,enabled,\`schedule\`,last,app_id,class,type) select 'b2c_tasks_sostatussync', 'U8定时任务_付款状态同步', 'false', '*/15 * * * *', NULL, 'b2c', 'b2c_tasks_sostatussync', 'custom' from dual where not exists (select 1 from \`sdb_base_crontab\` where id='b2c_tasks_sostatussync');" >>insert_predata.sh.log 2>&1
CheckStatus;
